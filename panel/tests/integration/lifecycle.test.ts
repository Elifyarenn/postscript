...
/** An admin, a main editor, an active writer and an article assigned to them. */
async function scenario() {
  const admin = await createUser({ role: "admin" });
  const editor = await createUser({ role: "editor" });
  const writer = await createUser({ role: "writer", writerStatus: "pending_agreement" });

  // The editorial chain (D-059) gives the first and second review stages to
  // editors; this test walks those with one main editor, so it is marked as one.
...
/**
 * Walks the whole review chain to `awaiting_rights`, where the approval opens:
 * the category editor approves (here the main editor covers the stage), the
 * main editor passes it to the admin, and the admin accepts it into the
 * publication flow, which auto-opens the Eser Onayı (D-059).
 */
...
describe("acceptance opens the work approval", () => {
  it("moves the article on and fixes the licence from the contract", async () => {
    ...
    const accepted = await toAwaitingRights(editor, admin, article.id);
  ...
  it("tells the writer an approval is waiting", async () => {
    const { admin, editor, writerRow, article } = await scenario();
...
describe("no publication without an approval", () => {
  it("refuses to schedule while the approval is pending, with 409", async () => {
    ...
    const error = await captureError(transitionArticle(admin, article.id, "scheduled", noMeta));
  ...
  it("schedules and publishes once the writer approves", async () => {
    const { admin, editor, writer, article } = await scenario();
    await toAwaitingRights(editor, admin, article.id);
    ...
    const scheduled = await transitionArticle(admin, article.id, "scheduled", noMeta);
...
describe("a changed text needs a new approval (§7.5)", () => {
  ...
  it("refuses to rewrite a published work in place", async () => {
    ...
    await approveAsWriter(writer, article.id);
    await transitionArticle(admin, article.id, "scheduled", noMeta);
    await transitionArticle(admin, article.id, "published", noMeta);
...
describe("withdrawal", () => {
  it("requires a reason and then answers 410 from the public API", async () => {
    ...
    const withoutReason = await captureError(
      transitionArticle(admin, article.id, "withdrawn", noMeta),
    );
    expect(withoutReason.status).toBe(409);

    await transitionArticle(admin, article.id, "withdrawn", noMeta, {
...
describe("scheduled publication job", () => {
  it("publishes what is due and leaves the rest alone", async () => {
    ...
    await transitionArticle(admin, article.id, "scheduled", noMeta, { scheduledAt: future });
